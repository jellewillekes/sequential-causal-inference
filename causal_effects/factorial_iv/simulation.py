...
print(all_data_wide.head())

# Define the formula for the analysis (pseudo code, as we do not have iv_factorial in Python)
# formula = 'team_rank ~ ' + ' + '.join([f'team_win_round{r}' for r in range(1, N_rounds + 1)]) + ' | ' + ' + '.join([f'team_better_round{r}' for r in range(1, N_rounds + 1)])
# For demonstration purposes, we will just print the formula
print('Formula:',
      'team_rank ~ ' + ' + '.join([f'team_win_round{r}' for r in range(1, N_rounds + 1)]) + ' | ' + ' + '.join(
          [f'team_better_round{r}' for r in range(1, N_rounds + 1)]))

# Save the wide format data to a CSV file
output_dir = "Germany"
os.makedirs(output_dir, exist_ok=True)
all_data_wide.to_csv(os.path.join(output_dir, f"cup_simulated_k{N_rounds}.csv"), index=False)
