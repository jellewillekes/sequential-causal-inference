...
# Function for loading processed data
...
def plot_logistic_regression(df, x_column, y_column, stage, country_plots_dir):
    ...
    plt.ylabel('Probability of Winning')
    plt.savefig(
        os.path.join(country_plots_dir, f'{x_column}_vs_probability_of_winning_logistic_regression_stage_{stage}.png'))
    plt.show()
...
def plot_avg_rank_by_stage_and_outcome(df, stage, country_plots_dir):
    ...
    plt.figure(figsize=(14, 8))
    sns.barplot(data=avg_opponent_rank_df, x='Match Outcome', y='Opponent_rank_prev', hue='Match Outcome',
